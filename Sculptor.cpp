...
// Ativa o voxel na posição (x,y,z) (fazendo show = true) e atribui ao mesmo a
...
void Sculptor::writeOFF(char const *filename) {
  ...
  // Calcule o número de Voxels ativos (show == true)
  for (int i = 0; i < nx; i++) {
    for (int j = 0; j < ny; j++) {
      for (int k = 0; k < nz; k++) {
        if (v[i][j][k].show) {
  ...
  for (int i = 0; i < nx; i++) {
    for (int j = 0; j < ny; j++) {
      for (int k = 0; k < nz; k++) {
        if (v[i][j][k].show) {
          outfile << std::fixed
                  << std::setprecision(2); // Use std::fixed e std::setprecision
